...
Node* addOwner(Node* root, const char* name, int fine, int depth) {
    ...
    if (strcmp(name, root->ownerName) < 0) {
        root->left = addOwner(root->left, name, fine, depth + 1);
    }
    else if (strcmp(name, root->ownerName) > 0) {
        root->right = addOwner(root->right, name, fine, depth + 1);
    }
    else {
...
Node* adjustFine(Node* root, const char* name, int fine, int* isRemoved, int depth) {
    ...
    if (strcmp(name, root->ownerName) < 0) {
        root->left = adjustFine(root->left, name, fine, isRemoved, depth + 1);
    }
    else if (strcmp(name, root->ownerName) > 0) {
        root->right = adjustFine(root->right, name, fine, isRemoved, depth + 1);
    }
    else {
        root->fineAmount -= fine;
        if (root->fineAmount <= 0) {
            if (!*isRemoved) {  // Ensure the message prints only once
                printf("%s removed\n", name);
                *isRemoved = 1;
            }
            if (!root->left) {
                ...
                root->left = adjustFine(root->left, maxNode->ownerName, maxNode->fineAmount, isRemoved, depth + 1);
            }
        }
        else {
            printf("%s %d %d\n", name, root->fineAmount, depth);
...
Node* findOwner(Node* root, const char* name, int depth) {
    ...
    if (strcmp(name, root->ownerName) < 0) {
        return findOwner(root->left, name, depth + 1);
    }
    else if (strcmp(name, root->ownerName) > 0) {
        return findOwner(root->right, name, depth + 1);
    }
    else {
        printf("%s %d %d\n", name, root->fineAmount, depth);
        return root;
...
void executeCommand(char command[], Node** root) {
    if (strcmp(command, "add") == 0) {
        ...
        int fine;
        scanf("%s %d", name, &fine);
        *root = addOwner(*root, name, fine, 0);
    }
    else if (strcmp(command, "deduct") == 0) {
        char name[MAX_NAME_LEN];
        int fine, isRemoved = 0;
        scanf("%s %d", name, &fine);
        *root = adjustFine(*root, name, fine, &isRemoved, 0);
    }
    else if (strcmp(command, "search") == 0) {
        char name[MAX_NAME_LEN];
        scanf("%s", name);
        findOwner(*root, name, 0);
    }
